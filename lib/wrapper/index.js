...
export default Radium(({ label, value, error, touch, name, Icon, icon, children, style, allowFloat = true }) => {
  const styles = {
    base: {
      width: '100%'
    , float: 'left'
  ...
  const colour = error && touch ? '#db2828' : (icon ? icon.props.colour : undefined);
