...
export default Radium(({ label, value, error, touch, name, Icon, icon, children, style, width = '100%', readOnly = false, allowFloat = true }) => {
  const styles = {
    base: {
      width: '100%'
    , float: 'left'
  ...
  return (
    <div style={styles.base}>
      ...
      <Error readOnly={readOnly} error={error} touch={touch} name={name} label={label} />
    </div>
  );
});