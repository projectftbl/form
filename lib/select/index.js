...
export default class Select extends Component {
  ...
  render() {
    const { value, error, icon, label = '', Icon } = this.props;

    const name = this.props.name ? this.props.name : label.toLowerCase().replace(' ', '');

    return (
      <Wrapper {...this.props} onClick={_ => this.click(name)}>
        <Field {...this.props} active={value} hasIcon={icon || Icon} />
      </Wrapper>
    );
  }
};
